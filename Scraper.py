...
class Scraper(Thread):
    ...
    def __init__(self, linkedin_username, linkedin_password, profiles_urls, headless=False):
        ...
        #     self.worksheet.write(0, h, headers[h])
    ...
    def run(self):
        ...
        if not self.browser.current_url == "https://www.linkedin.com/feed/":
            ...
            sleep_time = random.randint(2,120)
            print(f"Time sleep {sleep_time} s")
            time.sleep(random.randint(2,300))
            if not self.browser.current_url == "https://www.linkedin.com/feed/":
                print(self.browser.current_url)
                sleep_time = random.randint(2,300)
                print(f"Time sleep {sleep_time} s")
                time.sleep(random.randint(2,300))
                raise AuthenticationException()
        print("Scraping from profiles usls")
        count_Error = 0
        for linkedin_url in self.profiles_urls:
            try:
                ...
                print('Number profile',self.count_profile, end="\r")
                count_Error = 0
            except:
                print(f"URL error: {linkedin_url}. Pass")
                count_Error+=1
                if count_Error>=3:
                    print("URL error>3, so do you want to check again username and password?")
                    anwser = input()
                    if anwser == "Y" or anwser=="y":
                        print("Set up time sleep (s): ")
                        try:
    ...
    def scrape_profile(self, linkedin_url, waiting_time=30):
        ...
        try:
            ...
            print("Another try will be performed within 30 seconds...")
    ...
    def scrape_profile_name(self):
        return self.browser.execute_script(
            "return document.getElementsByClassName('display-flex justify-space-between pt2')[0].getElementsByClassName('pv-text-details__left-panel mr5')[0].children[0].innerText")
